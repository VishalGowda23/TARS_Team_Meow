...
async function testEncryption() {
    ...
    const keypair = await index_1.CryptoService.generateKeyPair();
    console.log("Test Keypair Generated.");
    // 2. Encrypt for the recipient
    console.log("Encrypting content...");
    const { encryptedFile, wrappedKeys } = await index_1.CryptoService.encryptForRecipients(fileBuffer, [keypair.publicKey]);
    ...
    const unwrappedKey = await index_1.CryptoService.unwrapKey(wrappedKeys[0], keypair.publicKey, keypair.privateKey);
    // 4. Decrypt the file
    console.log("Decrypting file...");
    const decryptedBuffer = index_1.CryptoService.decryptBuffer(encryptedFile, unwrappedKey);
