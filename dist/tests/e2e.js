...
async function runE2E() {
    ...
    try {
        // 1. Setup Identities
        const submitter = ethers_1.ethers.Wallet.createRandom();
        const validator = ethers_1.ethers.Wallet.createRandom();
        const recipient = ethers_1.ethers.Wallet.createRandom();
        ...
        if (!submissionRes.ok)
            throw new Error(`Submission failed: ${await submissionRes.text()}`);
        ...
        if (!voteRes.ok)
            throw new Error(`Vote failed: ${await voteRes.text()}`);
