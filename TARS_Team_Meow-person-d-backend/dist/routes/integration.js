...
async function integrationRoutes(server) {
    /**
     * POST /api/store
     *
     ...
     */
    server.post("/api/store", async (request, reply) => {
        const { stageId, evidenceHash, encryptionRequired } = request.body;
        // Validation
        ...
        server.log.info({ stageId, evidenceHash: evidenceHash.substring(0, 16) + "...", encryptionRequired }, "📥 Person B Integration: Storage request received");
        try {
            // ─────────────────────────────────────────────────────────────────
            ...
            if (calculatedHash !== evidenceHash) {
                server.log.error({ expected: evidenceHash, actual: calculatedHash }, "❌ HASH MISMATCH - Evidence may have been tampered with");
            ...
            if (encryptionRequired) {
                ...
                symmetricKey = (0, symmetric_1.generateSymmetricKey)();
                const encryptedData = (0, symmetric_1.encryptBuffer)(fileBuffer, symmetricKey);
            ...
            const cid = await (0, pinata_1.uploadToIPFS)(bufferToStore, `evidence-${stageId}`);
            ...
            try {
                ...
                arweaveId = await (0, bundlr_1.archiveToArweave)(bufferToStore);
                server.log.info({ arweaveId }, "✅ Arweave archival complete");
            }
            catch (arweaveError) {
                server.log.warn({ error: arweaveError.message }, "⚠️ Arweave archival failed (non-critical) - IPFS storage succeeded");
            }
            ...
            let evidence;
            try {
                if (!(0, prisma_1.isDatabaseConnected)()) {
                    throw new Error("Database not connected");
                }
                evidence = await prisma_1.default.evidence.create({
                    data: {
     ...
     */
    server.get("/api/store/status/:evidenceId", async (request, reply) => {
     ...
     */
    server.post("/api/store/verify", async (request, reply) => {
        const { cid, expectedHash } = request.body;
        if (!cid || !expectedHash) {
            return reply.status(400).send({ error: "Missing cid or expectedHash" });
        }
        try {
            const evidence = await prisma_1.default.evidence.findFirst({
                where: { ipfsCid: cid }
            });
            if (!evidence) {
