...
import { File, Clock, CheckCircle2, Radio, Copy, Tag, FileText, ExternalLink, Eye, Download, Scale } from 'lucide-react';
import { useState } from 'react';
import { auditApi } from '@/lib/api';
import Comments from '@/components/comments/Comments';
import LegalExportModal from '@/components/legal/LegalExportModal';
...
export default function EvidenceCard({ evidence, index, isSelected, onSelect }: EvidenceCardProps) {
    ...
    const [showLegalExport, setShowLegalExport] = useState(false);
    ...
    const viewFile = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (evidence.ipfsCid && evidence.ipfsCid !== 'Pending...') {
            // Open file in new tab via IPFS gateway
            window.open(`https://gateway.pinata.cloud/ipfs/${evidence.ipfsCid}`, '_blank');
        }
    };

    const viewOnEtherscan = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (evidence.txHash && evidence.txHash !== 'Pending...') {
            window.open(`https://sepolia.etherscan.io/tx/${evidence.txHash}`, '_blank');
        }
    };
    ...
    const hasValidCid = evidence.ipfsCid && evidence.ipfsCid !== 'Pending...';
    const hasValidTxHash = evidence.txHash && evidence.txHash !== 'Pending...';
    ...
    return (
      ...
      `}
        >
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {isSelected && (
                        ...
                        >
                            ...
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    window.open(auditApi.getDownloadUrl(evidence.id, 'text'), '_blank');
                                }}
                                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg
                    ...
                    {/* Comments Section */}
                    <Comments evidenceId={evidence.id} evidenceTitle={displayTitle} />
                </div>
            </div>
        ...
        {/* Legal Export Modal - Rendered outside the card to prevent glitching */}
        <LegalExportModal
            evidenceId={evidence.id}
            evidenceTitle={displayTitle}
            isOpen={showLegalExport}
            onClose={() => setShowLegalExport(false)}
        />
        </>
    );
}
